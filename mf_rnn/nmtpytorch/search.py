...
def beam_search(models, data_loader, task_id=None, beam_size=12, max_len=200,
    ...
    if task_id is None:
        ...
        decs = [m.dec for m in models]  #get_decoder(task.first_trg)
    ...
    for batch in pbar(data_loader, unit='batch'):
        ...
        ctx_dicts = models[0].dec.ctx_dict(ctx_dicts[0])
        ctx_dicts = [ctx_dicts]
        ...
        for tstep in range(max_len):
            ...
            log_ps, h_ts = zip(
                *[f_next(cd, dec.get_emb(idxs, tstep), h_t[tile]) for
                  f_next, dec, cd, h_t in zip(f_nexts, decs, ctx_dicts, h_ts)]) #,txt,video, hier

           # txt_list.append(txt[0].squeeze(1)), video_list.append(video[0].squeeze(1)), hier_list.append(hier[0].squeeze(1).squeeze(1))

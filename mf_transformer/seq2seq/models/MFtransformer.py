...
from seq2seq.modules import MultiheadAttention, LearnedPositionalEmbedding, HierarchicalAttention,CrossmodalFusion
...
@register_model('MFtransformer')
class MFTransformerModel(MMSeq2SeqModel):
    ...
    @classmethod
    def build_model(cls, args, all_dict):
        ...
        shared_embed_tokens = build_embedding(all_dict, args.decoder_embed_dim, args.decoder_embed_path)

        encoder = TransformerEncoder(args, all_dict, shared_embed_tokens)
        decoder = MFTransformerDecoder(args, all_dict, shared_embed_tokens)
        return MFTransformerModel(encoder, decoder)
...
class MFTransformerDecoder(Seq2SeqDecoder):
    def __init__(self, args, dictionary, embed_tokens):
        super().__init__(dictionary)
        embed_dim = embed_tokens.embedding_dim
        self.padding_idx = embed_tokens.padding_idx
        ...
        self.layers[0] = MFTransformerDecoderLayer(args)
        self.layer_norm = nn.LayerNorm(embed_dim)

        self.embed_out = nn.Parameter(torch.Tensor(len(dictionary), embed_dim))
        nn.init.kaiming_normal_(self.embed_out)   
        
        self.cross_fusion = CrossmodalFusion(2048,512)
    ...
    def forward(self, tgt_inputs, encoder_out, video_inputs, incremental_state=None):
        ...
        encoder_out['encoder_out'] = self.layer_norm(encoder_out['encoder_out'])
...
class MFTransformerDecoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.v_proj = nn.Linear(2048,self.embed_dim)
        nn.init.kaiming_normal_(self.v_proj.weight)
        nn.init.constant_(self.v_proj.bias, 0.)
        self.encoder_attn_v = MultiheadAttention(self.embed_dim, args.decoder_attention_heads, args.attention_dropout)
        self.ha_attn = HierarchicalAttention(self.embed_dim,self.embed_dim)
        self.vt_layer_norm = nn.LayerNorm(self.embed_dim)
    ...
    def forward(
        ...
        tx = F.dropout(tx, p=self.dropout, training=self.training)

        video_inputs = self.v_proj(video_inputs).transpose(0,1)
        vx, attnv = self.encoder_attn_v(
            query=x, key=video_inputs, value=video_inputs,incremental_state=incremental_state, static_kv=True, need_weights=(not self.training))
        vx = F.dropout(vx, p=self.dropout, training=self.training)
        tx = self.vt_layer_norm(tx) 
        vx = self.vt_layer_norm(vx) 
        _, x = self.ha_attn(hid=x,ctx1=tx,ctx2=vx) 
        x = F.dropout(x, p=self.dropout, training=self.training) 
        x = x + residual 
...
@register_model_architecture('MFtransformer', 'MFtransformer')
